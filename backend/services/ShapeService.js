...
const shapeService = {
  ...
  saveMultipleShape: async (req, res) => {
    ...
    try {
      ...
      const shapeId = parseInt(shapesData[0].shape_id);

      if (isNaN(shapeId)) {
        throw new Error("shape_id must be a valid integer");
      }
      ...
      const fields = validFields.filter(
        (field) => shapesData[0][field] !== undefined
      );
      fields.push("user_id");
      ...
      for (const shape of shapesData) {
        ...
        for (const field of fields) {
          if (field === "shape_id") {
            rowValues.push(shapeId);
          } else if (field === "user_id") {
            rowValues.push(user_id);
          } else {
            rowValues.push(shape[field]);
          }
        }
    ...
    `;
      ...
      await connection.execute(
        `UPDATE trips SET shape_id = ? WHERE trip_id = ? AND user_id = ?`,
